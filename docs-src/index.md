# AWS S3 SERVELESS APPLICATION | CLOUD COMPUTING - INSPER

- **Alunos:** Lister Ogusuku Ribeiro
...
Qando o terraform concluir suas etapas até aqui, podemos entrar no dashboard da AWS e encontrar um bucket S3 recém-criado com um nome definido por meio de um gerador de animais de estimação aleatório.

Note que dentro do bucket são armazenadas funções lambdas dentro de um zip.

Qaundo entramos na dashboard da AWS CloudWatch também conseguimos ver o grupo de logs criado.
...
Também precisamos conceder permissões ao API Gateway para invocar nossa funçõ lambda:
...
Se entrarmos no dashboard do CloudWatch, conseguiremos ver os logs de acesso registrados para cada solicitação.
